...
- **`translate` / `translate review` / `translate markdown` need OpenAI (or an OpenAI-compatible API).** These alpha commands call the AI SDK with `OPENAI_API_KEY` and optionally `OPENAI_BASE_URL` / `--base-url` / config `openai.baseUrl` for compatible providers; `validate`, `scan`, and `ci`/`init-ci` run fully offline. `translate markdown` is a single-file prototype (`--from`, `--to`, `--out`); it does not walk folders.
